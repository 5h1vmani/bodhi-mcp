...
Patterns verified across Anthropic, Apple, Microsoft Fluent 2, and Google M3 (Jan 2026):

### Universal (4/4 companies)

| Pattern                              | Implementation                                                                                                                                                                                                    | Evidence                                                                                                                                                                            |
| ------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| CSS custom properties as token layer | All 4 use CSS variables as the runtime token interface; Anthropic: `--_theme---*`; Fluent: global + alias layers; M3: W3C DTCG                                                               | This is the settled answer for runtime theming -- no build step needed for theme switching                                                                                          |
| Two-font-family strategy             | Anthropic: Copernicus (serif display) + sans (UI); Apple: SF Pro Display + SF Pro Text (optical sizes); Google: Google Sans (display) + Google Sans Text (body) | Display fonts optimize for impact at large sizes; text fonts optimize for readability at small sizes; context-specific fonts are a valid third pattern |
| Context-specific typography          | Every company uses different type treatments per product context (marketing vs product, display vs data, dashboard vs focus task)                                                                                 | Not just font pairing — whole typographic postures shift by context; the `data-layer` attribute approach is a clean implementation                                             |
| Reduced motion respect               | All 4 implement `prefers-reduced-motion: reduce`                                                                                                           | Non-negotiable accessibility requirement; implementation varies from graceful degradation to full disable                                                                           |
| Neutral-first palette                | Anthropic: slate/cloud swatches; Apple: pure white/black; M3: surface/on-surface; Fluent: neutral tokens                                                                                  | Background and structure colors are always neutral; brand/accent is the minority (<10-30%)                                                                                          |

### Near-Universal (4/5 companies)

| Pattern                     | Implementation                                                                                                     | Evidence                                                                                                                           |
| --------------------------- | ------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------- |
| Fixed signals + brand split | Semantic signals (success=green, error=red, warning=amber) are never tenant-customizable; only brand accent varies | The 90/10 rule (90% locked, 10% brand) is the clearest formulation; Microsoft and Google confirm via alias token architecture |
| 4px base grid               | Fluent: explicit 4px `designUnit`; M3: 4dp grid; Apple: even spacing                    | Industry standard; 8px layout grid is built on 4px atoms; use 4px for component internals, 8px for layout                          |
| Stagger animation on lists  | Anthropic: page-load stagger; Apple: iOS list reveals; M3: stagger delays        | 50ms per-item delay is the sweet spot; the CSS custom property approach is clean and performant                               |
| Tabular numbers for data    | Apple SF, Google Sans, Fluent all enable `font-feature-settings: "tnum" 1` for numeric data                | Prevents layout shift in counters, tables, metrics; specify in CSS, not just font selection                                        |

### Emerging (2-3/5 but trending)

| Pattern                           | Implementation                                                                                                                                                 | Evidence                                                                                                                                                                          |
| --------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Spring-based easing               | M3 Expressive: physics engine (stiffness + damping ratio); Apple Liquid Glass: spring animations | M3 Expressive replaces duration-based motion entirely with spring physics; two spring types: spatial (position — can overshoot) and effects (opacity — no overshoot)              |
...
| 2026-01-30 | Add: Industry Pattern Convergence section (Anthropic, Apple, Microsoft Fluent 2, Google M3 cross-reference); 5 universal patterns, 4 near-universal, 3 emerging; 5 new references |
